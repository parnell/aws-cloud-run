...
class TestParseECRImageUri:
    ...
    def test_non_ecr_image_returns_none(self):
        ...
        assert parse_ecr_image_uri("python:3.11") is None
        assert parse_ecr_image_uri("nginx:latest") is None
        assert parse_ecr_image_uri("public.ecr.aws/docker/library/python:3.12") is None
        assert parse_ecr_image_uri("gcr.io/project/image:tag") is None

    def test_invalid_ecr_uri_returns_none(self):
        """Invalid ECR URIs return None."""
        assert parse_ecr_image_uri("") is None
        assert parse_ecr_image_uri("not-a-uri") is None
...
class TestHumanReadableTime:
    ...
    def test_just_now(self):
        ...
        result = human_readable_time(now)
    ...
    def test_minutes_ago(self):
        ...
        assert "minute" in human_readable_time(one_min_ago)

        # 30 minutes ago
        thirty_min_ago = now - timedelta(minutes=30)
        result = human_readable_time(thirty_min_ago)
    ...
    def test_hours_ago(self):
        ...
        assert "hour" in human_readable_time(one_hour_ago)

        # 5 hours ago
        five_hours_ago = now - timedelta(hours=5)
        assert "hour" in human_readable_time(five_hours_ago)
    ...
    def test_handles_naive_datetime(self):
        ...
        aware_dt = datetime.now(UTC) - timedelta(minutes=5)
        naive_dt = aware_dt.replace(tzinfo=None)
        result = human_readable_time(naive_dt)
        assert "minute" in result
