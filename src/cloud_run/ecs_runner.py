...
def resolve_vpc_and_subnets(
    ...
    if vpc_name_or_id.startswith("vpc-"):
        ...
        # Search by name tag
        vpcs = ec2.describe_vpcs(Filters=[{"Name": "tag:Name", "Values": [vpc_name_or_id]}])
        if not vpcs["Vpcs"]:
    ...
    if public_subnets:
        ...
        print(
            f"[cloud_run] Using {len(selected)} private subnet(s) (may need NAT gateway)",
            file=sys.stderr,
        )
        return selected
    else:
    ...
    for vpc in vpcs["Vpcs"]:
        ...
        if not subnets["Subnets"]:
            ...
            for subnet in subnets["Subnets"]:
                ...
                internet = (
                    subnet_internet.get(subnet_id) or subnet_internet.get(f"main:{vpc_id}") or "✗"
                )
                ...
                print(
                    f"      {subnet_id}  {az}  public:{public_ip}  internet:{internet}  {subnet_name}"
                )
...
def list_task_definitions(region: str, prefix: str | None = None) -> None:
    ...
    for family in sorted(families):
        try:
            ...
            print(
                f"{status} {family:<48} {revision:<6} {cpu:<6} {memory:<6} {container_count:<12} {image}"
            )
        except Exception:
...
def _resolve_ecs_config(
        ...
        if cluster_arn:
            ...
            if task_defs:
                ...
                if usable:
                    ...
                    print(
                        f"  cloud_run <script> --ecs --cluster {cluster_name} --task-definition {suggested}\n",
                        file=sys.stderr,
                    )
                else:
                    print(
                        "[cloud_run] No task definitions without warnings found.", file=sys.stderr
                    )
                    print(
                        "  You can still use one with --task-definition (warnings will be shown).",
                        file=sys.stderr,
                    )
                    print(
                        "  Or omit --cluster to let cloud_run create a new task definition.\n",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"[cloud_run]   Task definition: {config.task_family} (will create)",
                    file=sys.stderr,
                )
    ...
    resolved_subnets: list[str] | None = None
    resolved_security_groups: list[str] | None = None
    ...
    # Then try VPC
        ...
        try:
            ...
            print(
                f"[cloud_run]   Warning: Could not resolve VPC {vpc_name_or_id}: {e}",
                file=sys.stderr,
            )
    ...
    if resolved_subnets:
        ...
        print(
            f"[cloud_run]   Subnets ({network_source}): {', '.join(resolved_subnets)}",
            file=sys.stderr,
        )
        if resolved_security_groups:
            print(
                f"[cloud_run]   Security groups: {', '.join(resolved_security_groups)}",
                file=sys.stderr,
            )
    else:
        raise RuntimeError(
...
def run_on_ecs(
    ...
    try:
        ...
        if config.needs_new_task_def:
            ...
            print(
                f"[cloud_run]   Registered task definition: {config.task_def_arn}", file=sys.stderr
            )
        ...
        if runtime_secrets:
            print(
                f"[cloud_run] Runtime secrets (fetched in container): {runtime_secrets}",
                file=sys.stderr,
            )
        ...
        logs = get_task_logs(
            config.log_group, task_id, container_name=config.container_name, region_name=region
        )
